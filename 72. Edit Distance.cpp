...
public:
    ...
    int solve(int i,int j,string &word1,string &word2){
          if(j<0) return i+1; //word2 traversed completely
          if(i<0) return j+1; //word1 traversed completely
    ...
    int solve2(int i,int j,string &word1,string &word2,vector<vector<int>>&dp){
          ...
          int insertion = 1 + solve2(i,j-1,word1,word2,dp) ;
          int deletion = 1 + solve2(i-1,j,word1,word2,dp) ;
          int replace = 1 + solve2(i-1,j-1,word1,word2,dp) ;
    ...
    //Recursion call from (n,m) -> After right shifting of index(index=n (right shifted) indicates index=n-1 in old scenario)
     int solve3(int i,int j,string &word1,string &word2,vector<vector<int>>&dp){
          if(j==0) return i; //word2 traversed completely
          if(i==0) return j; //word1 traversed completely
          ...
          int insertion = 1 + solve3(i,j-1,word1,word2,dp) ;
          int deletion = 1 + solve3(i-1,j,word1,word2,dp) ;
          int replace = 1 + solve3(i-1,j-1,word1,word2,dp) ;
