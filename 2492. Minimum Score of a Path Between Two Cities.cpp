...
public:
    int minScore(int n, vector<vector<int>>& roads) {
         ...
         //find the component in which both 1&n are present in that find least wt of edge is the answer 
         //Apply bfs or dfs or union-find all work

        DisjointSet ds(n); //1 based indexing already handle as parent,rank,size of size n+1 
        int ans=INT_MAX;
        //make dynamic connections -> disjoint set bein created
        for(auto&it : roads){
            ds.UnionBySize(it[0],it[1]);
        }
        //now,check if belonging to same component(Same ultimate Parent) take min wt of edge 
        for(auto&it: roads){ 
            //1 and it[0] or 1 and it[1] should belong to same component
            //as it[0] -> it[1] this means both connected to 1 if any one connected to 1
            if(ds.findUltimateParent(1) == ds.findUltimateParent(it[0])){
               ans=min(ans,it[2]);
            }
        }
        return ans;
