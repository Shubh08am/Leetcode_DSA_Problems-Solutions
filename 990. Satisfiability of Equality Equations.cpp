...
public:
    bool equationsPossible(vector<string>& equations) {
         DisjointSet ds(26);//26 as a to z all character
         
        for(auto it : equations){
            int xi = it[0]-'a' , yi = it[3]-'a' ; 
            if(it[1]=='=') { 
              //  ds.UnionByRank(xi,yi);
                ds.UnionBySize(xi,yi);
                }
        }
            
        for(auto it : equations){
                int xi = it[0]-'a' , yi = it[3]-'a' ;
             //if same ultimate parent and not equal to relation
             if(it[1]=='!'){
                if(ds.findUltimateParent(xi)==ds.findUltimateParent(yi)){
                return false;
                }
            }
        }
